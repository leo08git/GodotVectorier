...
@export_tool_button("Export level to Vector") var toolbutton_compilelevel = func(): await (await LevelManager.CreateFromLevel(self)).Compile()
@export_tool_button("Copy level content") var toolbutton_toclipboard = func(): (await LevelManager.CreateFromLevel(self)).ToClipboard()
#endregion
...
## Level sets, I recommend leaving this as the default value.
@export var sets: PackedStringArray = [
	"buildings.xml" ,
	"buildings_construction.xml" ,
	"buildings_downtown.xml" ,
	"buildings_lab.xml" ,
	"buildings_techpark.xml" ,
	"buildings_techpark_bonus.xml" ,
	"objects.xml" ,
	"objects_construction.xml" ,
	"objects_downtown.xml" ,
	"objects_lab.xml" ,
	"objects_techpark.xml" ,
]
...
## If enabled, whenever a [Sprite2D] is created, automatically set the script to the image class script.
@export var sprite2d_script_set: bool = true
## Automatically set sprite2D's pivot to top left when starting when enabled - Recommended.
@export var sprite2d_fix_pivot: bool = true
#endregion
#endregion
