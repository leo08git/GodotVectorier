...
# Parsing raw objects.
	for object_dictionary in objects:
		...
		elif UnityComponent.COMPONENTS.has(object_type) and not from_prefab:
...
# Resolving prefabs
	for prefabdata in PendingPrefabs:
		...
		push_warning("[UnityScene] Found prefab in scene \"%s\", as of now, there's no support for prefabs." % ScenePath)
